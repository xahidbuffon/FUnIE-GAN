...
def netG4_decoder(layers, reuse=False):
   sc = tf.get_variable_scope()
   with tf.variable_scope(sc, reuse=reuse):
      enc_conv1 = layers[0]
      enc_conv2 = layers[1]
      dec_conv1 = tcl.convolution2d_transpose(enc_conv2, 64, 4, 2, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_conv1')
      dec_conv1 = relu(dec_conv1)
      dec_conv1 = tf.concat([dec_conv1, enc_conv1], axis=3)
      print 'dec_conv1:',dec_conv1
      dec_conv2 = tcl.convolution2d_transpose(dec_conv1, 3, 4, 2, activation_fn=tf.nn.tanh, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_conv4')
...
def netG8_encoder(x):
   ...
   embedding = tcl.fully_connected(tcl.flatten(enc_conv3), 512, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_enc_embedding')
   ...
   print
   layers = [enc_conv1, enc_conv2, enc_conv3, embedding]
   return layers
...
def netG8_decoder(layers, reuse=False):
   ...
   with tf.variable_scope(sc, reuse=reuse):
      ...
      enc_conv3 = layers[2]
      embedding = layers[3]
      embedding = tcl.fully_connected(embedding, 32*32*256, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_embedding')
      embedding = tf.reshape(embedding, [-1, 32, 32, 256])
      embedding = tf.concat([embedding, enc_conv3], axis=3)
      print 'embedding:',embedding
      dec_conv1 = tcl.convolution2d_transpose(embedding, 128, 4, 2, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_conv1')
      dec_conv1 = relu(dec_conv1)
      dec_conv1 = tf.concat([dec_conv1, enc_conv2], axis=3)
      print 'dec_conv1:',dec_conv1
      dec_conv2 = tcl.convolution2d_transpose(dec_conv1, 64, 4, 2, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_conv2')
      dec_conv2 = relu(dec_conv2)
      dec_conv2 = tf.concat([dec_conv2, enc_conv1], axis=3)
      print 'dec_conv2:',dec_conv2
      dec_conv3 = tcl.convolution2d_transpose(dec_conv2, 3, 4, 2, activation_fn=tf.nn.tanh, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_conv4')
      print 'dec_conv3:',dec_conv3
      return dec_conv3

def netG10_encoder(x):
   enc_conv1 = tcl.conv2d(x, 64, 4, 2, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_enc_conv1')
   enc_conv1 = tcl.batch_norm(enc_conv1)
   enc_conv1 = lrelu(enc_conv1)
   enc_conv2 = tcl.conv2d(enc_conv1, 128, 4, 2, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_enc_conv2')
   ...
   embedding = tcl.fully_connected(tcl.flatten(enc_conv4), 512, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_enc_embedding')
   embedding = tcl.batch_norm(embedding)
   embedding = lrelu(embedding)
   print 'x:        ',x
   print 'enc_conv1:',enc_conv1
   ...
   print 'embedding:',embedding
   print
   layers = [enc_conv1, enc_conv2, enc_conv3, enc_conv4, embedding]
   return layers

def netG10_decoder(layers, reuse=False):
   sc = tf.get_variable_scope()
   with tf.variable_scope(sc, reuse=reuse):
      enc_conv1 = layers[0]
      enc_conv2 = layers[1]
      ...
      enc_conv4 = layers[3]
      embedding = layers[4]
      embedding = tcl.fully_connected(embedding, 16*16*512, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_embedding')
      embedding = tf.reshape(embedding, [-1, 16, 16, 512])
      embedding = tf.concat([embedding, enc_conv4], axis=3)
      print 'embedding:',embedding
      dec_conv1 = tcl.convolution2d_transpose(embedding, 256, 4, 2, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_conv1')
      dec_conv1 = relu(dec_conv1)
      dec_conv1 = tf.concat([dec_conv1, enc_conv3], axis=3)
      print 'dec_conv1:',dec_conv1
      dec_conv2 = tcl.convolution2d_transpose(dec_conv1, 128, 4, 2, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_conv2')
      dec_conv2 = relu(dec_conv2)
      dec_conv2 = tf.concat([dec_conv2, enc_conv2], axis=3)
      print 'dec_conv2:',dec_conv2
      dec_conv3 = tcl.convolution2d_transpose(dec_conv2, 64, 4, 2, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_conv3')
      dec_conv3 = relu(dec_conv3)
      dec_conv3 = tf.concat([dec_conv3, enc_conv1], axis=3)
      print 'dec_conv3:',dec_conv3
      dec_conv4 = tcl.convolution2d_transpose(dec_conv3, 3, 4, 2, activation_fn=tf.nn.tanh, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_dec_conv4')
      print 'dec_conv4:',dec_conv4
      return dec_conv4

def netG12_encoder(x):
   enc_conv1 = tcl.conv2d(x, 64, 4, 2, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_enc_conv1')
   enc_conv1 = tcl.batch_norm(enc_conv1)
   enc_conv1 = lrelu(enc_conv1)
   enc_conv2 = tcl.conv2d(enc_conv1, 128, 4, 2, activation_fn=tf.identity, weights_initializer=tf.random_normal_initializer(stddev=0.02), scope='g_enc_conv2')
...
def netG12_decoder(layers, reuse=False):
   sc = tf.get_variable_scope()
   with tf.variable_scope(sc, reuse=reuse):
      enc_conv1 = layers[0]
      enc_conv2 = layers[1]
